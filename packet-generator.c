...
int main(void) {
	...
	if (ppcap == NULL) {
		printf("Could not open interface mon0 for packet injection: %s",
				errbuf);
		return 2;
	}
	...
	while (1) {
		...
		sz = sizeof(u8aRadiotapHeader) + sizeof(struct ieee80211_hdr)
				...
				+ /*0*/sizeof(uint8_t) + sizeof(struct timespec) + 100 /* data */
				+ 4 /* FCS */;
